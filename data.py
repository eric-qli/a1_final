...
if os.environ.get('SOLN', False):
    ...
    from q2_algorithm_soln import isProjective
    from q1_parse_soln import PartialParse, get_sentence
else:
    from q2_algorithm import isProjective
...
class Transducer(object):
    ...
    def pp2feat(self, pp):
        ...
        for stack_idx in range(min(3, len(pp.stack))):
            ...
            for l_idx, l_dep in enumerate(
                    pp.getNLeftMost(sentence_idx, n=2)):
                ...
                if not l_idx:  # leftmost-leftmost
                    for ll_dep in pp.getNLeftMost(l_dep, n=1):
            ...
            for r_idx, r_dep in enumerate(
                    pp.getNRightMost(sentence_idx, n=2)):
                ...
                if not r_idx:  # rightmost-rightmost
                    for rr_dep in pp.getNRightMost(r_dep, n=1):
    ...
    def graphs2feats_and_tds(self, graphs):
        ...
        It uses PartialParses' getOracle method to determine the
...
class UDData(Dataset):
    ...
    def parse_etc(self, s: str) -> T.Tuple[T.List[str, ...], T.Tuple[int, ...],
        ...
        forms, heads, deprels = zip(*[(t['form'], t['head'],
                                      ...
                                      for t in sen])
        return forms, heads, deprels, isProjective(heads)
