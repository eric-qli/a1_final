...
from configs import Q1Config, Q2Config
from q1_parse import minibatchParse
from q1_model import ParserModel


if os.environ.get('SOLN', False):
    sys.path.append("../solution")
    from q1_parse_soln import minibatchParse
    from q1_model_soln import ParserModel
    from q2_model_soln import GraphDepModel
else:
    from q1_parse import minibatchParse
    from q1_model import ParserModel
    from q2_model import GraphDepModel
...
@main.command()
@click.pass_context
@click.option('--model-weight-path', type=Path, default='weights-q1.pt')
@click.option('--dev-output-path', type=Path, default='dev.q1_out.conllu')
@click.option('--test-output-path', type=Path, default='test.q1_out.conllu')
def q1(ctx, model_weight_path, dev_output_path, test_output_path):
    '''Run Q1 model.'''
    ...
    def run_and_save(sentences, filepath, desc):
        ...
        pred_deps = minibatchParse(sentences, model, model.config.batch_size)
...
@main.command()
@click.pass_context
@click.option('--model-weight-path', type=Path, default='weights-q2.pt')
@click.option('--dev-output-path', type=Path, default='dev.q2_out.conllu')
@click.option('--test-output-path', type=Path, default='test.q2_out.conllu')
def q2(ctx, model_weight_path, dev_output_path, test_output_path):

    dir_path = ctx.obj['path']

    cfg = Q2Config()
    cfg.data_dir = dir_path / 'corpora'
    cfg.model_dir = './transformers'
